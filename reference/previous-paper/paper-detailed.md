...
| specialized | Docker layer cache | `cargo-chef` (Docker/CI only) | dependency-layer caching in container builds |
...
| Codex | 1,891,748 | 46% | 20% | 18% | 16% | ~54% |
...
2. The shape is **carrier-local**: almost all of Cursor's waste rides one carrier (analytics chatter), almost all of Codex's rides another (plugin catalog plus retry storms), and almost all of Antigravity's rides a third (opening-turn environment-context replay).
3. The projected band is smallest where the task lane is cleanest (Antigravity at 12%, Codex at 16%) and largest where the task lane is hidden behind control-plane noise (Cursor at 59% under shadow caveat).
...
These rules are why the same five cascade stages present uniformly across Claude (Anthropic Messages), Codex (ChatGPT backend), Cursor (`aiserver.v1` gRPC-web), and Antigravity (Gemini `/v1internal`).
