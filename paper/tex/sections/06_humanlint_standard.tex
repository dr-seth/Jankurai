\section{humanlint Standard and Conformance}
...
The adoption loop is itself part of the standard: `humanlint init` installs the control plane, `humanlint doctor` reports drift, and `humanlint audit` produces the proof ledger. A repository that only mentions the standard but does not install or execute it is not conformant.

\begin{table}[t]
\caption{humanlint conformance levels.}
