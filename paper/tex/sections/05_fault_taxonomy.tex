...
\end{tabularx}
\end{minipage}
\caption{TLR means Top-Level Risk. Shares are computed directly from Table~\ref{tab:fault-taxonomy}: each row contributes its RPN to one TLR bucket, then bucket totals are rounded to whole percentages. The chart is a policy-priority model for humanlint 0.4.0, not an incident-frequency measurement.}
\label{fig:tlr-pie}
\end{figure*}
