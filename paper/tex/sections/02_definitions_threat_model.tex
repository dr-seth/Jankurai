...
The paper uses operational definitions rather than rhetorical labels. \emph{Vibe coding} is any development mode where plausible code is accepted without explicit ownership, proof routing, generated-boundary checks, security gates, and repair evidence. \emph{Agent-native engineering} is the inverse: the repository is structured so an agent or human reviewer can discover the owner, edit the smallest legal surface, run the mapped proof lane, and leave evidence. A \emph{repository standard} is the versioned set of normative rules, artifacts, schemas, proof lanes, and governance states a repository claims and an auditor can test. A \emph{conformance artifact} is a machine-readable file or generated report needed to verify that claim. A \emph{proof lane} is a deterministic command or CI job tied to a path or change class. A \emph{continuous audit} is the local, pull-request, and CI loop that reruns those checks at merge time; it is not assumed to be a hosted daemon. A \emph{proof ledger} is the appendable set of proof plans, command receipts, evidence indexes, audit reports, and score history rows that allow a later agent to verify what was proven. A \emph{repair queue} is an ordered list of findings with owners, lanes, rerun commands, evidence requirements, and bounded next actions. A \emph{generated zone} is an output path that is repaired by changing its source contract and regenerating. A \emph{repair receipt} is the durable record of what failed, why, what was changed, and which proof accepted the repair. \emph{Governed human code} is any human-authored change entering a repository that claims the same owner, proof, audit, and security controls as agent-authored code. \emph{Centerline drift} is the distance between the repository's actual structure and the standard it claims to follow.
