...
| VCOV-02 | Canonical coverage source | done | `agent/vibe-coverage.toml` | `jankurai vibe validate` | source registry | taxonomy names are mechanically seeded |
...
| VCOV-04 | Vibe CLI parser/reporter | done | `crates/jankurai/src/commands/vibe.rs`, CLI wiring | `cargo test -p jankurai --test vibe_coverage_smoke` | JSON/Markdown/TeX reports | table length may need paper tuning |
...
| VCOV-06 | Rule registry extension | done | `audit/rules.rs`, scan/findings, tests, docs | `cargo test -p jankurai --test rule_registry_smoke` | rule registry export | detectors remain heuristic |
...
| VCOV-09 | Version sweep | done | version manifests, changelog, docs | `just versions` | 0.6.0 bindings | missed version snippet |
