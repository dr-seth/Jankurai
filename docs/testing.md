...
Tool adoption is scored separately from the core proof lanes. The built-in catalog currently tracks `audit-ci`, `proof-routing`, `security`, `ux-qa`, `db-migration-analyze`, `contract-drift`, and `rust-witness`. `agent/tool-adoption.toml` records per-tool mode as `auto`, `required`, `advisory`, or `disabled`. In the audit report, a tool only counts as replaced when CI runs the relevant Jankurai-backed lane and uploads the expected artifact evidence; local config is readiness only.
