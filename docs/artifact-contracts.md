...
| `agent/vibe-coverage.toml` | `schemas/vibe-coverage-source.schema.json`; `vibe_coverage_smoke`, `schema_contracts` | Canonical source-row mapping for `tips/vibe_coding/*.txt` |
| `target/jankurai/vibe-coverage.json` | `schemas/vibe-coverage-report.schema.json`; `vibe_coverage_smoke` | Emitted coverage report with counts by coverage, TLR, rule, tool, and priority |
