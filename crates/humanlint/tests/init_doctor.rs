...
fn init_dry_run_writes_nothing() {
    ...
    assert!(!dir.path().join("target/humanlint/receipts").exists());
}
...
fn init_yes_is_idempotent_for_existing_root_guidance() {
    ...
    assert_eq!(agents.matches("humanlint merge marker").count(), 1);
    assert!(dir.path().join(".cursor/rules/humanlint.mdc").exists());
...
fn init_dry_run_plan_json_is_machine_readable() {
    let dir = tempdir().unwrap();
    let plan = dir.path().join("target/humanlint/init-plan.json");
...
#[test]
fn init_yes_keeps_existing_humanlint_guidance_without_marker() {
    let dir = tempdir().unwrap();
    fs::write(
    ...
    let agents = fs::read_to_string(dir.path().join("AGENTS.md")).unwrap();
    assert!(!agents.contains("humanlint merge marker"));
}
...
"#,
    ...
    assert!(Command::new(env!("CARGO_BIN_EXE_humanlint"))
        ...
        .success());
    let workflow =
        fs::read_to_string(ci_dir.path().join(".github/workflows/humanlint.yml")).unwrap();
