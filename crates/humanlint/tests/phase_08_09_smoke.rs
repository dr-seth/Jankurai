...
fn context_pack_command_writes_pack_and_markdown() {
    ...
    let json_out = dir.path().join("target/humanlint/context-pack.json");
    let md_out = dir.path().join("target/humanlint/context-pack.md");
    assert!(Command::new(env!("CARGO_BIN_EXE_humanlint"))
        .arg("context-pack")
        .arg(dir.path())
        .arg("--task")
        .arg("repair agent context routing")
        .arg("--changed")
        .arg("agent/HUMANLINT_STANDARD.md")
        .arg("--out")
        .arg(&json_out)
        .arg("--md")
        .arg(&md_out)
    ...
    assert!(fs::read_to_string(md_out)
        .unwrap()
        .contains("# humanlint Context Pack"));
}
...
fn repair_plan_command_writes_packets() {
    ...
    let report_path = dir.path().join("target/humanlint/repo-score.json");
    let report = json!({
        "findings": [{
            "severity": "high",
            "category": "security",
    ...
    let out = dir.path().join("target/humanlint/repair-plan.json");
    let md = dir.path().join("target/humanlint/repair-plan.md");
    assert!(Command::new(env!("CARGO_BIN_EXE_humanlint"))
        .arg("repair-plan")
        .arg(dir.path())
        .arg("--from")
    ...
    assert!(fs::read_to_string(md)
        .unwrap()
        .contains("# humanlint Repair Plan"));
}
...
fn seed_catalog(repo: &std::path::Path) {
    ...
    fs::write(
        ...
        r#"{"tests":{"agent/":{"command":"cargo test -p humanlint","purpose":"agent checks"},"docs/":{"command":"cargo run -p humanlint -- . --json agent/repo-score.json --md agent/repo-score.md","purpose":"audit"}}}"#,
    )
    .unwrap();
    fs::write(
...
command = "cargo run -p humanlint -- . --json agent/repo-score.json --md agent/repo-score.md"
read_only = false
"#,
    )
    .unwrap();
