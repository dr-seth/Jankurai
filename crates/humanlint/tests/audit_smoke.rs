...
use humanlint::model::ProofReceipt;
use humanlint::render::render_markdown;
use humanlint::report::{issues, junit, sarif};
...
fn audit_yaml_echo_is_not_proof() {
    ...
    fs::write(dir.path().join("Justfile"), "check:\n    cargo test\n").unwrap();
    fs::write(
        dir.path().join(".github/workflows/humanlint.yml"),
...
fn audit_self_audit_includes_tool_internals() {
    ...
    .unwrap();
    fs::write(dir.path().join("Justfile"), "check:\n    cargo test\n").unwrap();
    fs::create_dir_all(dir.path().join("crates/humanlint/src")).unwrap();
    fs::write(
        dir.path().join("crates/humanlint/src/lib.rs"),
    ...
    assert!(!default_report
        .findings
        .iter()
        .any(|finding| finding.path == "crates/humanlint/src/lib.rs"));
    assert!(self_report
        .findings
        .iter()
        .any(|finding| finding.path == "crates/humanlint/src/lib.rs"));
...
fn audit_rule_ids_resolve_to_registry_entries() {
    ...
    fs::write(dir.path().join("README.md"), "# thin repo\n").unwrap();
    let report = run_audit(dir.path(), &[]).unwrap();
    let registry: HashSet<&'static str> = humanlint::audit::rule_registry()
        .iter()
        .map(|rule| rule.id)
        .collect();
...
fn markdown_renders_proof_receipts() {
    ...
    report.proof_receipts.push(ProofReceipt {
        ...
        exit_code: 0,
        elapsed_ms: 12,
        artifacts: vec!["target/humanlint/proof-plan.json".into()],
        changed_paths: vec![],
        owner: None,
        skipped_reason: None,
        residual_risk: vec![],
