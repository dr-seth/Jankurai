...
pub struct ProveArgs {
    ...
    /// When set with `HUMANLINT_ALLOW_UNSIGNED_PROOF_COMMANDS=1`, run commands not in proof-lanes/test-map.
...
pub fn run_prove(args: ProveArgs) -> Result<()> {
    ...
    let log_dir = args.repo.join("target/humanlint/logs");
    ...
    let evidence = ProofEvidenceIndex {
        ...
        sarif_path: optional_repo_relative_existing(&args.repo, "target/humanlint/humanlint.sarif"),
...
pub fn build_proof_plan(
        ...
        if let Some((prefix, spec)) = catalog.test_route_for_path(path) {
            ...
            let entry = planned_runs
                ...
                .or_insert_with(|| PlannedRun {
                    ...
                    artifacts: vec![
                        "target/humanlint/logs/*.log".to_string(),
                        "target/humanlint/proof-receipts/*.json".to_string(),
                    ],
    ...
    let expected_artifacts = vec![
        "target/humanlint/proof-plan.json".to_string(),
        "target/humanlint/proof-plan.md".to_string(),
        "target/humanlint/proof-receipts/*.json".to_string(),
        "target/humanlint/logs/*.log".to_string(),
        "target/humanlint/evidence-index.json".to_string(),
    ];
...
fn ensure_planned_commands_allowed(
    ...
    let env_ok = std::env::var("HUMANLINT_ALLOW_UNSIGNED_PROOF_COMMANDS")
        .map(|v| v == "1")
        .unwrap_or(false);
        ...
        if !allow.contains(&normalized) {
            anyhow::bail!(
                "proof command not in agent proof-lanes or test-map allowlist: `{}`\n\
                 ...
                 or pass --allow-unsigned-commands with HUMANLINT_ALLOW_UNSIGNED_PROOF_COMMANDS=1",
                run.command
            );
        }
    }
...
fn render_markdown(plan: &ProofPlan) -> String {
    ...
    let _ = writeln!(out, "# humanlint Proof Plan");
    let _ = writeln!(out);
