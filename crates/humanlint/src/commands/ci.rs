...
pub fn install(args: CiInstallArgs) -> Result<()> {
    if !args.github {
        bail!("only `humanlint ci install --github` is currently supported");
    }
    let path = args.repo.join(".github/workflows/humanlint.yml");
  ...
  audit:
    ...
    steps:
      ...
      - name: humanlint audit
        run: cargo run -p humanlint -- audit . --mode {mode} --json agent/repo-score.json --md agent/repo-score.md --sarif target/humanlint/humanlint.sarif --github-step-summary target/humanlint/summary.md --repair-queue-jsonl target/humanlint/repair-queue.jsonl
