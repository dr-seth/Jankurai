...
pub fn run(args: DoctorArgs) -> Result<()> {
    ...
    check_json_artifact(
        &repo,
        &mut diagnostics,
        "target/humanlint/context-pack.json",
    ...
    check_json_artifact(
        &repo,
        &mut diagnostics,
        "target/humanlint/repair-plan.json",
...
fn render_markdown(diagnostics: &[Diagnostic]) -> String {
    let mut out = String::from("# humanlint doctor\n\n");
    for diagnostic in diagnostics {
        out.push_str(&format!(
            "- **{}** `{}` `{}`: {}\n",
...
fn push_file_check(repo: &Path, diagnostics: &mut Vec<Diagnostic>, rel: &str) {
    if repo.join(rel).exists() {
        ...
        diagnostics.push(Diagnostic {
            ...
            severity: "high".into(),
            path: rel.into(),
            message: "missing required humanlint control file".into(),
        });
    }
}
...
fn check_proof_ledger(repo: &Path, diagnostics: &mut Vec<Diagnostic>) {
    let receipts_dir = repo.join("target/humanlint/proof-receipts");
    ...
    let index_path = repo.join("target/humanlint/evidence-index.json");
...
fn check_security_evidence(repo: &Path, diagnostics: &mut Vec<Diagnostic>) {
    let evidence_path = repo.join("target/humanlint/security/evidence.json");
...
fn check_receipt_exports(repo: &Path, diagnostics: &mut Vec<Diagnostic>) {
    if !repo.join("target/humanlint").exists() {
        diagnostics.push(Diagnostic {
            check_id: "receipt-export".into(),
            severity: "low".into(),
            path: "target/humanlint".into(),
