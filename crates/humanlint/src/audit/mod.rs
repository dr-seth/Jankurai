...
pub fn run_audit_with_options(
    ...
    let mut report = Report {
        ...
        hard_rules: CAPS
            ...
            .collect(),
        dimensions,
        ux_qa: attach_ux_report_artifact(root, analyzers::ux_qa_status(&ctx)),
...
fn build_findings(
    ...
    if caps_applied.contains(&"no-humanlint-audit-lane-in-ci".into()) {
        b.add("high", "audit", ".github/workflows", "CI does not run the humanlint audit lane", "add a CI job that runs `humanlint . --json agent/repo-score.json --md agent/repo-score.md` and uploads both artifacts", vec!["audit output must stay JSON plus Markdown for agent repair routing".into()], None, None);
    }
    ...
    for hit in scan::manifest_parse_findings(ctx) {
        b.add(
            ...
            "audit",
            &hit.path,
            "humanlint manifest could not be parsed",
    ...
    if caps_applied.contains(&"missing-rendered-ux-qa-lane".into()) {
        b.add("high", "ux-qa", "apps/web", "web surface lacks layered rendered UX QA evidence", "add Storybook state coverage, Playwright screenshots, visual review or `@humanlint/ux-qa`, accessibility scans, CLS checks, generated mocks, and design tokens", vec!["rendered UX QA lane missing".into()], Some("HLT-013-RENDERED-UX-GAP"), None);
    }
