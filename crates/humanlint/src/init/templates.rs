...
const ADAPTER_POINTER: &str = "<!-- humanlint generated adapter -->\nRead `AGENTS.md` first. Use `agent/HUMANLINT_STANDARD.md` as the canonical humanlint standard.\nFor MASTER_PLAN work, read `agent/MASTER_PLAN.md`, then `tips/phases/00-phase-index.md`, then the active `tips/phases/*.md` phase file. Log phase work in `tips/phases/logs/`.\nFor planning work, follow `agent/MASTER_PLAN.md#detailed-planner-protocol`.\n";
const PROOF_ADAPTER_POINTER: &str = "# humanlint\n\n<!-- humanlint generated adapter -->\nRead `AGENTS.md` first. Use `agent/HUMANLINT_STANDARD.md` as the canonical humanlint standard.\nFor MASTER_PLAN work, read `agent/MASTER_PLAN.md`, then `tips/phases/00-phase-index.md`, then the active `tips/phases/*.md` phase file. Log phase work in `tips/phases/logs/`.\nFor planning work, follow `agent/MASTER_PLAN.md#detailed-planner-protocol`.\nRun the proof lane in `agent/test-map.json` for changed paths.\n";

pub const TEMPLATES: &[Template] = &[
    Template {
        path: "AGENTS.md",
        body: "# Agent Instructions\n\nRead `agent/HUMANLINT_STANDARD.md` first. For phase or MASTER_PLAN work, read `agent/MASTER_PLAN.md` before `tips/phases/00-phase-index.md`. Keep generated artifacts under their declared source commands.\n",
    },
    Template {
        path: ".cursor/rules/humanlint.mdc",
        body: "---\nalwaysApply: true\n---\n\n<!-- humanlint generated adapter -->\nRead `AGENTS.md` first. Use `agent/HUMANLINT_STANDARD.md` as the canonical humanlint standard.\nFor MASTER_PLAN work, read `agent/MASTER_PLAN.md`, then `tips/phases/00-phase-index.md`, then the active `tips/phases/*.md` phase file. Log phase work in `tips/phases/logs/`.\nFor planning work, follow `agent/MASTER_PLAN.md#detailed-planner-protocol`.\n",
    },
    Template {
        path: "CLAUDE.md",
    ...
    Template {
        path: ".github/instructions/humanlint.instructions.md",
        body: ADAPTER_POINTER,
    },
    Template {
        path: ".github/instructions/humanlint-rust.instructions.md",
        body: "---\napplyTo: \"**/*.rs\"\n---\n\n<!-- humanlint generated adapter -->\nRead `AGENTS.md` first. Use `agent/HUMANLINT_STANDARD.md` as the canonical humanlint standard.\nFor MASTER_PLAN work, read `agent/MASTER_PLAN.md`, then `tips/phases/00-phase-index.md`, then the active `tips/phases/*.md` phase file. Log phase work in `tips/phases/logs/`.\nFor planning work, follow `agent/MASTER_PLAN.md#detailed-planner-protocol`.\n",
    },
    Template {
        path: ".github/instructions/humanlint-web.instructions.md",
        body: "---\napplyTo: \"**/*.{ts,tsx,js,jsx,css}\"\n---\n\n<!-- humanlint generated adapter -->\nRead `AGENTS.md` first. Use `agent/HUMANLINT_STANDARD.md` as the canonical humanlint standard.\nFor MASTER_PLAN work, read `agent/MASTER_PLAN.md`, then `tips/phases/00-phase-index.md`, then the active `tips/phases/*.md` phase file. Log phase work in `tips/phases/logs/`.\nFor planning work, follow `agent/MASTER_PLAN.md#detailed-planner-protocol`.\n",
    },
    Template {
        path: ".github/instructions/humanlint-python-ai.instructions.md",
    ...
    Template {
        ...
        body: "# Install humanlint\n\nRun `humanlint init --profile rust-ts-postgres --ide all --mode advisory --dry-run`, review the plan, then rerun with `--yes`.\n",
    },
    Template {
    ...
    Template {
        ...
        body: "# IDE Integrations\n\nAll IDE adapters are thin pointers to `agent/HUMANLINT_STANDARD.md`; keep durable policy there or in `docs/`.\n",
    },
    Template {
        path: "docs/exceptions/README.md",
        body: "# humanlint Exceptions\n\nDocument dated exceptions with owner, expiry, migration path, and proof lane.\n",
    },
    Template {
        path: "README-humanlint-scaffold.md",
        body: "# Greenfield scaffold (non-production)\n\nThis tree was bootstrapped with `humanlint init --profile rust-ts-postgres`. Replace this file with a real product README when you have one.\n",
    },
    Template {
    ...
    Template {
        ...
        body: "#!/usr/bin/env bash\nset -euo pipefail\n# Scaffold stub: replace with real secret/dependency/SBOM checks (see humanlint `tools/security-lane.sh` in the standard repo).\necho \"security-lane scaffold: ok\"\n",
    },
    Template {
        path: "agent/ux-qa.toml",
        body: "artifact_root = \"target/humanlint/ux-qa\"\n",
    },
    Template {
        path: ".github/workflows/humanlint.yml",
        body: "name: humanlint\n\non:\n  pull_request:\n  push:\n    branches: [main]\n\njobs:\n  audit:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      - uses: dtolnay/rust-toolchain@stable\n      - uses: actions/setup-node@v4\n        with:\n          node-version: \"22\"\n          cache: npm\n      - run: npm ci\n      - run: cargo run -p humanlint -- versions\n      - name: humanlint audit\n        run: cargo run -p humanlint -- audit . --mode ratchet --json agent/repo-score.json --md agent/repo-score.md --sarif target/humanlint/humanlint.sarif --github-step-summary target/humanlint/summary.md --repair-queue-jsonl target/humanlint/repair-queue.jsonl\n      - name: Enforce score floor\n        run: test \"$(jq -r '.score' agent/repo-score.json)\" -ge 85\n      - name: Security lane\n        run: bash tools/security-lane.sh\n      - uses: actions/upload-artifact@v4\n        with:\n          name: humanlint-score\n          path: |\n            agent/repo-score.json\n            agent/repo-score.md\n            target/humanlint/humanlint.sarif\n            target/humanlint/repair-queue.jsonl\n",
    },
];
