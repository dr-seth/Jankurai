...
pub fn render_junit(report: &Report) -> String {
    ...
    let _ = writeln!(
        out,
        r#"<testsuite name="humanlint" tests="{}" failures="{}">"#,
        failures.max(1),
        failures
    );
    if report.findings.is_empty() {
        let _ = writeln!(out, r#"  <testcase classname="humanlint" name="score"/>"#);
    } else {
        for finding in &report.findings {
            ...
            let _ = writeln!(out, r#"  <testcase classname="humanlint" name="{}">"#, name);
            let _ = writeln!(
                out,
