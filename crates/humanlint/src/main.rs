...
use humanlint::audit::policy::AuditMode;
use humanlint::audit::{run_audit, run_audit_with_options, AuditOptions};
use humanlint::commands::{
...
use humanlint::report::issues::IssueFormat;
...
#[derive(Parser, Debug)]
#[command(name = "humanlint", version, args_conflicts_with_subcommands = true)]
struct Cli {
    #[command(subcommand)]
...
struct ProveArgs {
    ...
    /// Requires `HUMANLINT_ALLOW_UNSIGNED_PROOF_COMMANDS=1` in the environment.
...
struct SecurityRunArgs {
    ...
    #[arg(
        long,
        value_name = "PATH",
        default_value = "target/humanlint/security/evidence.json"
    )]
    out: String,
    #[arg(long)]
    strict: bool,
    ...
    match cli.command {
        ...
        Some(Commands::Ci { command }) => match command {
            CiCommand::Install(args) => {
                humanlint::commands::ci::install(humanlint::commands::ci::CiInstallArgs {
                    repo: args.repo,
                    github: args.github,
                    mode: args.mode,
...
fn run_audit_and_write(args: AuditArgs) -> anyhow::Result<()> {
    ...
    let changed = if let Some(base) = args.changed_from.as_deref() {
        humanlint::audit::changed_paths_from_git(&args.repo, base)?
    } else {
        args.changed
    };
    ...
    if matches!(mode, AuditMode::Release) && report.proof_receipts.is_empty() {
        ...
        humanlint::audit::rebuild_agent_fix_queue(&mut report);
        if let Some(decision) = report.decision.as_mut() {
    ...
    report.report_fingerprint = humanlint::audit::report_fingerprint(&report);
    ...
    if let Some(path) = args.sarif.as_deref() {
        write_json(path, &humanlint::report::sarif::render_sarif(&report))?;
    }
    if let Some(path) = args.junit.as_deref() {
        write_json(path, &humanlint::report::junit::render_junit(&report))?;
    }
    if let Some(path) = args.github_step_summary.as_deref() {
        write_markdown(
            path,
            &humanlint::report::github::render_step_summary(&report),
        )?;
    }
    if let Some(path) = args.repair_queue_jsonl.as_deref() {
        write_json(
            path,
            &humanlint::report::issues::repair_queue_jsonl(&report),
        )?;
    }
    eprintln!(
...
fn run_adapters_verify(args: AdapterVerifyArgs) -> anyhow::Result<()> {
    let failures = humanlint::init::adapters::verify_adapters(&args.repo)?;
...
fn run_adapters_sync(args: AdapterSyncArgs) -> anyhow::Result<()> {
    ...
    let plan = humanlint::init::adapters::adapter_plan(&args.repo, &args.ide);
    for action in &plan {
        println!("{} {}", action.action, action.path);
    }
    if args.yes {
        humanlint::init::adapters::write_adapters(&args.repo, &args.ide, false)?;
    }
    Ok(())
}

fn run_explain(rule_id: &str) -> anyhow::Result<()> {
    let Some(doc) = humanlint::audit::docs_for_rule_id(rule_id) else {
...
fn run_issues_export(args: IssuesExportArgs) -> anyhow::Result<()> {
    ...
    let text = humanlint::report::issues::render_issues(&report, format);
...
fn apply_mode_and_baseline(
    report: &mut humanlint::model::Report,
    mode: AuditMode,
    baseline: Option<&str>,
) -> anyhow::Result<()> {
    if let Some(policy) = report.policy.as_mut() {
    ...
    if let Some(decision) = report.decision.as_mut() {
        ...
        if let Some(path) = baseline {
            ...
            decision.ratchet = Some(humanlint::model::ReportRatchet {
                baseline_score,
                allowed_drop: 0,
...
fn run_ux_passthrough() -> anyhow::Result<()> {
    ...
    if status.success() {
        ...
        anyhow::bail!("humanlint ux exited with {}", status)
    }
}
